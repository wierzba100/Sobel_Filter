...
module top(
    ...
    delay #(4) u_delay (
        .clk(clk),
        .rst(reset),
        .in_signal(pxl_out_gray_normalize),
        .out_signal(delayed_reg)
    );
...
module delay (
    ...
    output logic [7:0] out_signal
);
    parameter DELAY_CYCLES = 4;

    logic [7:0] shift_reg[DELAY_CYCLES-1:0];

    always @(posedge clk or posedge rst) begin
        if (rst) begin
            for(int i = 0; i < DELAY_CYCLES; i=i+1)
            begin
                shift_reg[i] <= 0;
            end
        end else begin
    ...
    assign out_signal = shift_reg[DELAY_CYCLES-1];
